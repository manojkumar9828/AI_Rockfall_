...
Creates a small synthetic dataset of procedural images representing mine
slope textures for each class (Safe, Warning, Dangerous), trains the CNN,
and saves the model to model/rockfall_model.h5. Lets the project run
end-to-end without requiring a real rockfall image dataset.
...
def train_on_synthetic():
    ...
    train_gen = aug.flow_from_directory(
        str(DATASET_DIR), target_size=(TRAIN_IMG_SIZE, TRAIN_IMG_SIZE),
        batch_size=4, class_mode="categorical", subset="training",
        classes=CLASS_NAMES, seed=42)
