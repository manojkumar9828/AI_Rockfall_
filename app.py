...
DEFAULT_SETTINGS = {"threshold": 70.0, "sensitivity": "medium", "theme": "dark"}
...
def create_app():
    ...
    def inject_globals():
        return {"current_user": session.get("username"), "current_role": session.get("role"), "settings": get_settings(), "now": datetime.now()}
    ...
    def prediction():
        ...
        try:
            ...
            app.logger.error("Prediction failed: %s", e, exc_info=True)
            flash("Prediction failed. The model may be unavailable or the image is invalid. Please try again.", "danger")
            return redirect(url_for("upload"))
        pred_id = database.save_prediction(session["user_id"], filename, f"uploads/{filename}", result["class_name"], result["confidence"], result["risk_level"], result["prediction_time"])
        alert_type, alert_msg = alert_for_prediction(result["class_name"])
        database.create_alert(pred_id, alert_type, alert_msg)
        database.log_action(session["user_id"], "predict", f"Predicted {filename}: {result['class_name']} ({result['confidence']}%)")
    ...
    def export_csv():
        ...
        for p in preds:
            writer.writerow([p["id"], p["image_name"], p["prediction"], p["confidence"], p["risk_level"], p["created_date"], p["created_time"]])
        output = si.getvalue()
        si.close()
        from flask import Response
        return Response(output, mimetype="text/csv", headers={"Content-Disposition": "attachment; filename=rockfall_history.csv"})
    ...
    def reports():
        ...
        if request.method == "POST":
            ...
            if report_type == "daily": start, end = today, today
            elif report_type == "weekly": start, end = today - timedelta(days=6), today
            else: start, end = today.replace(day=1), today
            preds = database.get_predictions()
            ...
            report_data = {"type": report_type, "start": start.strftime("%Y-%m-%d"), "end": end.strftime("%Y-%m-%d"), "total": total, "safe": safe, "warning": warning, "danger": danger, "accuracy": accuracy}
            database.save_report(session["user_id"], report_type, start.strftime("%Y-%m-%d"), end.strftime("%Y-%m-%d"), report_data)
    ...
    def settings():
        if request.method == "POST":
            session["settings"] = {"threshold": float(request.form.get("threshold", 70)), "sensitivity": request.form.get("sensitivity", "medium"), "theme": request.form.get("theme", "dark")}
    ...
    def forbidden(e):
        return render_template("error.html", code=403, message="Access forbidden. Admin privileges required."), 403
